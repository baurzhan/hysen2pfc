...
HYSEN_MODE_TO_HASS = {
    HYSEN_2PFC_MODE_FAN  : STATE_FAN_ONLY,
    HYSEN_2PFC_MODE_COOL : STATE_COOL,
    HYSEN_2PFC_MODE_HEAT : STATE_HEAT,
}

HASS_MODE_TO_HYSEN = {
    STATE_FAN_ONLY : HYSEN_2PFC_MODE_FAN,
    STATE_COOL     : HYSEN_2PFC_MODE_COOL,
    STATE_HEAT     : HYSEN_2PFC_MODE_HEAT,
}

HYSEN_FAN_TO_HASS = {
    HYSEN_2PFC_FAN_LOW    : SPEED_LOW,
    HYSEN_2PFC_FAN_MEDIUM : SPEED_MEDIUM,
    HYSEN_2PFC_FAN_HIGH   : SPEED_HIGH,
    HYSEN_2PFC_FAN_AUTO   : STATE_AUTO,
}

HASS_FAN_TO_HYSEN = {
    SPEED_LOW    : HYSEN_2PFC_FAN_LOW,
    SPEED_MEDIUM : HYSEN_2PFC_FAN_MEDIUM,
    SPEED_HIGH   : HYSEN_2PFC_FAN_HIGH,
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info = None):
    ...
    hysen_device = Hysen2PipeFanCoilDevice((host, 80), mac_addr, HYSEN_2PFC_DEV_TYPE, timeout)
    ...
    async def async_service_handler(service):
        ...
        if entity_ids:
            target_hvacrs = [dev for dev in hass.data[DATA_KEY].values()
...
class Hysen2PipeFanCoil(ClimateDevice):
    ...
    def device_state_attributes(self):
        ...
        if self._device_available:
            attr.update({
                ATTR_KEY_LOCK: str(HYSEN_KEY_LOCK_TO_HASS[self._hysen_device.key_lock]),
                ...
                ATTR_HYSTERESIS: str(HYSEN_HYSTERESIS_TO_HASS[self._hysen_device.hysteresis]),
                ...
                ATTR_FAN_CONTROL: str(HYSEN_FAN_CONTROL_TO_HASS[self._hysen_device.fan_control]),
    ...
    def operation_list(self):
        ...
        if self.current_fan_mode == STATE_AUTO:
            return OPERATION_MODES_AUTO
        else:
    ...
    def target_temperature(self):
        ...
        if self._hysen_device.power_state:
            if self.current_operation == STATE_FAN_ONLY:
                return None
    ...
    def supported_features(self):
        ...
        if self._hysen_device.power_state:
            if self.current_operation == STATE_FAN_ONLY:
                return SUPPORT_ON_OFF | SUPPORT_FAN_MODE | SUPPORT_OPERATION_MODE
            else:
                return SUPPORT_ON_OFF | SUPPORT_FAN_MODE | SUPPORT_OPERATION_MODE | SUPPORT_TARGET_TEMPERATURE
        else:
    ...
    async def async_set_hysteresis(self, hysteresis):
        ...
        if hysteresis.lower() not in HYSTERESIS_MODES:
            _LOGGER.error("[%s] Error in async_set_hysteresis. Unknown hysteresis \'%s\'.",
                self._host,
                hysteresis)
            return
        await self._try_command(
